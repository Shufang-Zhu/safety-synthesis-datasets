...
def tlsf_gen_real_1_env(k):
    ...
    for i in range(0,k+1):
        ...
        if(i==k):
            print('G( X(c',str(i), ') || u)', sep="", file=out)
        else:
            print('G( X(c',str(i), ') && ', sep="", file=out)
    for i in range(0,k):
        print(')', end = '', file=out)
    print('}\n}\n', file=out)
    # end 'print GUARANTEE'
...
def tlsf_gen_real_1_conj(k):
    filename = 'Dataset_seeds/scalable-benchmarks/real_1/real_scalable_1_'+str('{0:03}'.format(k+1))+'.conjtlsf'
    out = open(filename, 'w')
    # 'print INFO'
    print('INFO {', file=out)
    print('TITLE:       \"Scalable Realizable Benchmark n.'+str(k)+ '\"', file=out)
...
def main(argv):
    ...
    for i in range(int(sindex),int(eindex)):
        if _format == "smv":
            ...
            tlsf_gen_real_1(i);
            tlsf_gen_real_1_env(i);
            tlsf_gen_real_1_conj(i);
        else:
            sys.exit()
