...
def tlsf_gen_unreal_2(k):
    filename = 'unreal_2_tlsf/unreal_scalable_2_'+str('{0:03}'.format(k+1))+'.tlsf'
    out = open(filename, 'w')
    # 'print INFO'
    print('INFO {', file=out)
    ...
    for i in range(0,k+1):
        ...
        print('( u',str(i), ' || u',str(i+1), ')' , end="", sep="", file=out)
        if(i<k):
          print(' && ', file=out)
        else:
          print(' ', file=out)
...
def smv_gen_unreal_2(k):
    filename = 'unreal_2_smv/unreal_scalable_2_'+str('{0:03}'.format(k+1))+'.smv'
    out = open(filename, 'w')
    if(k<0):
        print('*** The index must be >= 0.');
        sys.exit()
...
def main(argv):
    ...
    for i in range(int(sindex),int(eindex)):
        if _format == "smv":
            print('Generating file unreal_scalable_2_', str('{0:03}'.format(i+1)), '.smv',sep='')
            smv_gen_unreal_2(i);
        elif _format == "tlsf":
            print('Generating file unreal_scalable_2_', str('{0:03}'.format(i+1)), '.tlsf',sep='')
            tlsf_gen_unreal_2(i);
        else:
            sys.exit()
